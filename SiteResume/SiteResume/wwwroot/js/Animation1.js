...
function drawLight() {
    ...
    gradient.addColorStop(0, "#192119");
...
function Box() {
    ...
    this.rotate = function() {
        var speed = (60 - this.half_size) / Speed;
        this.r += speed * 0.002;
        this.x += speed;
        this.y += speed;
    }
	...
	for (var i = boxes.length - 1; i >= 0; i--) {
		if(i != b){	
			...
			if (d < boxes[b].half_size + boxes[i].half_size) {
                boxes[b].half_size = boxes[b].half_size > 1 ? ++boxes[b].half_size : ++boxes[b].half_size;
                boxes[i].half_size = boxes[i].half_size > 1 ? --boxes[i].half_size : ++boxes[i].half_size;
			}
		}
	}
}
